...
export async function POST(request: Request) {
  try {
    ...
    if (items.length > 10) {
      return NextResponse.json({ error: "Trop d'articles dans la requete." }, { status: 400 })
    }
    ...
    const lineItems = items.map((item) => {
      ...
      if (!Number.isInteger(item.peopleCount) || item.peopleCount < 1) {
        throw new Error(`BAD_REQUEST: Nombre de personnes invalide pour ${item.id}`)
      }
      if (item.peopleCount > pkg.availablePlaces) {
        throw new Error(`BAD_REQUEST: Quantite superieure aux places disponibles pour ${item.id}`)
      }
    ...
    if (message.startsWith("BAD_REQUEST:")) {
      return NextResponse.json({ error: message.replace("BAD_REQUEST:", "").trim() }, { status: 400 })
    }
